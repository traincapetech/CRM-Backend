...
const getMaskedSettings = (settings) => {
  ...
  if (masked.apiKey) {
    masked.maskedApiKey = masked.apiKey.length > 8 
...
exports.updateBiometricSettings = async (req, res) => {
  ...
  try {
    ...
    const { enabled, vendorName, apiBaseUrl, apiKey, authType, webhookSecret, syncIntervalMinutes } = req.body;
  ...
  try {
    ...
    try {
      ...
      return res.status(200).json({
        ...
        data: { receivedLogs: Array.isArray(testPayload) ? testPayload.length : 0 }
      });
    } catch (error) {
...
exports.diagnose = async (req, res) => {
  ...
  try {
    ...
    const employee = await Employee.findOne({ biometricCode }).select('_id fullName email biometricCode biometricEnabled userId');
    
    // Also try with normalized code (leading zeros removed)
    const normalizedCode = biometricCode.replace(/^0+/, '');
    const employeeByNormalized = normalizedCode !== biometricCode 
      ? await Employee.findOne({ biometricCode: normalizedCode }).select('_id fullName email biometricCode biometricEnabled userId')
      : null;
    
    // Find recent biometric logs - try both exact and normalized
    const logQuery = normalizedCode !== biometricCode 
    ...
    const recentLogs = await BiometricLog.find(logQuery)
      .sort({ eventTime: -1 })
      .limit(10)
      .select('eventTime eventType attendanceDate employeeId vendorLogId biometricCode');
    
    // Find recent attendance records
    const employeeId = employee?._id || employeeByNormalized?._id;
    const recentAttendance = employeeId ? await Attendance.find({ employeeId })
      .sort({ date: -1 })
      .limit(10)
    ...
    const uniqueCodes = [...new Set(allRecentLogs.map(log => log.biometricCode))];
    
    // For each unique code, find which employee it belongs to
    const codeToEmployeeMap = {};
    for (const code of uniqueCodes) {
      const emp = await Employee.findOne({ biometricCode: code }).select('fullName email biometricCode');
      if (emp) {
        ...
        if (normalized !== code) {
          const empNorm = await Employee.findOne({ biometricCode: normalized }).select('fullName email biometricCode');
          if (empNorm) {
            codeToEmployeeMap[code] = {
            ...
            codeToEmployeeMap[code] = { unmatched: true, normalizedCode: normalized };
          }
        } else {
    ...
    const isEnabled = employee?.biometricEnabled || employeeByNormalized?.biometricEnabled || false;
    
    return res.status(200).json({
      success: true,
      data: {
        biometricCode,
        normalizedCode: normalizedCode !== biometricCode ? normalizedCode : null,
        employee: employee ? {
          id: employee._id,
