...
exports.sendCampaign = async (req, res) => {
  try {
    ...
    if (campaign.recipientType === 'all' || campaign.recipientType === 'leads') {
      ...
      const allLeads = await Lead.find(query).select('name email course').lean();
      ...
      const sales = await Sale.find({}).select('customerName email');
      recipients = sales
      ...
      const segmentLeads = await Lead.find(leadQuery).select('name email course country createdAt').lean();
      ...
      for (let i = 0; i < recipients.length; i += batchSize) {
        ...
        for (const recipient of batch) {
          try {
            ...
            htmlContent = htmlContent.replace(/\{\{name\}\}/g, recipient.name || 'Customer');
            htmlContent = htmlContent.replace(/\{\{email\}\}/g, recipient.email);
            htmlContent = htmlContent.replace(/\{\{course\}\}/g, recipient.course || '');
            htmlContent = htmlContent.replace(/\{\{country\}\}/g, recipient.country || '');
