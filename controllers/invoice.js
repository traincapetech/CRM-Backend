...
exports.createInvoice = async (req, res) => {
  try {
    ...
    // Convert invoiceDate string to Date if provided, otherwise use current date
    if (req.body.invoiceDate) {
      req.body.invoiceDate = new Date(req.body.invoiceDate);
    }

    // Calculate due date based on invoice date (or current date if not provided)
    if (req.body.paymentTerms && req.body.paymentTerms !== 'Due on Receipt') {
      const days = parseInt(req.body.paymentTerms.split(' ')[1]) || 30;
      const baseDate = req.body.invoiceDate || new Date();
      req.body.dueDate = new Date(baseDate.getTime() + days * 24 * 60 * 60 * 1000);
    }
...
exports.updateInvoice = async (req, res) => {
  try {
    ...
    if (req.body.paymentTerms && req.body.paymentTerms !== 'Due on Receipt') {
      const days = parseInt(req.body.paymentTerms.split(' ')[1]) || 30;
      const baseDate = req.body.invoiceDate || invoice.invoiceDate || new Date();
      req.body.dueDate = new Date(baseDate.getTime() + days * 24 * 60 * 60 * 1000);
    }
