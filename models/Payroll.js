...
const payrollSchema = new mongoose.Schema(
  {
    ...
    paymentMethod: {
      type: String,
      enum: ["BANK_TRANSFER", "CASH", "CHEQUE", "PAYTM_BANK", "PAYTM_UPI"],
      default: "BANK_TRANSFER",
    },

    // Paytm Payout Integration (migrated from Razorpay)
    // Migration Note: razorpayPayoutId, razorpayPayoutStatus, razorpayPayoutUTR, razorpayPayoutFees replaced
    paytmTransactionId: {
      type: String,
      default: null,
    },
...
payrollSchema.methods.calculateSalary = function () {
  console.log(
    "🧮 Starting fully manual salary calculation for payroll:",
    this._id,
  );
  ...
  console.log(
    "✅ Formula: Basic + All Allowances + All Bonuses - All Deductions = Net Salary",
  );
  console.log(
    `✅ ${this.basicAmount} + ${(this.hra || 0) + (this.da || 0) + (this.conveyanceAllowance || 0) + (this.medicalAllowance || 0) + (this.specialAllowance || 0) + (this.overtimeAmount || 0) + (this.reimbursements || 0)} + ${(this.performanceBonus || 0) + (this.projectBonus || 0) + (this.attendanceBonus || 0) + (this.festivalBonus || 0)} - ${this.totalDeductions} = ${this.netSalary}`,
  );
