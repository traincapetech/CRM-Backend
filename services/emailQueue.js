...
const queueEmails = async (recipients, campaignId, subject, htmlContent, plainText, batchSize = 50, delayBetweenBatches = 1000) => {
  ...
  for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
    ...
    for (const recipient of batch) {
      ...
      personalizedHtml = personalizedHtml.replace(/\{\{name\}\}/g, recipient.name || 'Customer');
      personalizedHtml = personalizedHtml.replace(/\{\{email\}\}/g, recipient.email);
      personalizedHtml = personalizedHtml.replace(/\{\{course\}\}/g, recipient.course || '');
      personalizedHtml = personalizedHtml.replace(/\{\{country\}\}/g, recipient.country || '');

      personalizedPlainText = personalizedPlainText.replace(/\{\{name\}\}/g, recipient.name || 'Customer');
      personalizedPlainText = personalizedPlainText.replace(/\{\{email\}\}/g, recipient.email);
      personalizedPlainText = personalizedPlainText.replace(/\{\{course\}\}/g, recipient.course || '');
      personalizedPlainText = personalizedPlainText.replace(/\{\{country\}\}/g, recipient.country || '');
